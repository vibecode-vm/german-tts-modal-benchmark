...
## 💰 Tatsächliche Modal-Kosten (Live aus `modal billing report`)

**Gesamt-Burn am 2026-05-18:** **$1.6834 USD** (~ €1.5487)

Da Modal Starter-Credits **$30/Monat gratis** sind, sind die tatsächlichen out-of-pocket Kosten: **$0.00**.

| App | Kosten USD | % vom Total | Was passierte |
|---|---:|---:|---|
| **xtts-de-tts** | $1.4499 | 86.1% | Größtes Image (PyTorch + TTS + DeepSpeed), 2.3 GB Modell, 124s Cold-Start |
| **orpheus-de-tts** | $0.1162 | 6.9% | 3 fehlgeschlagene Deploy-Versuche (annotations / SNAC / gated repo) |
| **thorsten-de-tts** | $0.0385 | 2.3% | Sauberer Deploy + 1 Cold-Start + 15 Samples |
| **mms-de-tts** | $0.0285 | 1.7% | Sauberer Deploy + 1 Cold-Start + 15 Samples |
| **magpie-de-tts** | $0.0191 | 1.1% | 3× crash-loop (NeMo-Versions-Mismatch), 3 Build-Attempts |
| **voice-agent-de** | $0.0162 | 1.0% | Live Push-to-Talk Agent (Whisper + Piper) |
| **piper-de-tts** | $0.0151 | 0.9% | Erfolgreich + 2× neu rendert (Voice-Fix kerstin → thorsten) |
| **TOTAL** | **$1.6834** | 100% | |

### Lessons Learned (Kosten-Hits)

- **86% des Burns floss in 1 App** (XTTS-v2) wegen großem Image + langer Modell-Download. Klein-Modelle wie Piper (~$0.015) und MMS (~$0.029) sind 50× günstiger.
- **Crash-Loops sind teuer**: Magpie hatte 3 Crash-Loops (~$0.02) ohne dass je ein Sample generiert wurde. **Lesson:** `scaledown_window=2` + Container-Status früh checken, sonst läuft Re-Try-Loop minutenlang.
- **Gated HF-Repos kostet auch ohne Erfolg**: Orpheus 3 Versuche = $0.12 ohne Output.
- **`min_containers=0` ist Pflicht** für Pay-per-Use. Wäre `=1` gewesen, hätten 4 GPU-Apps × 24h = ~$240/Tag gekostet.

### Was es gekostet hätte ohne Optimierungen

| Szenario | Geschätzte 24h-Kosten |
|---|---:|
| **4 GPU-Apps warm (`min_containers=1`)** | ~$200/Tag (3× L4 @ $0.80/h + 1× L40S @ $1.95/h) |
| **Voice-Agent immer warm** | ~$1.13/Tag (CPU 4-core × $0.047/h) |
| **Unser Setup (pay-per-use, alles gestoppt nach Tests)** | **$1.68 für die gesamte Test-Session** |

[Raw billing data](reports/modal_billing_2026-05-18.json)

---

*Generated by [`build_report.py`](client/build_report.py) from `reports/benchmark_results.json` + `reports/modal_billing_2026-05-18.json`.*
