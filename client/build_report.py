...
def main():
    ...
    # === Real billing breakdown ===
    billing_file = REPORTS / "modal_billing_2026-05-18.json"
    if billing_file.exists():
        bdata = json.loads(billing_file.read_text())
        by_app = {}
        for r in bdata:
            n = r["Description"]
            by_app[n] = by_app.get(n, 0) + float(r["Cost"])
        total = sum(by_app.values())
        P("## 💰 Tatsächliche Modal-Kosten (Live aus `modal billing report`)")
        P("")
        P(f"**Gesamt-Burn am 2026-05-18:** **${total:.4f} USD** (~ €{total*0.92:.4f})")
        P("")
        P("Da Modal Starter-Credits **$30/Monat gratis** sind, sind die tatsächlichen out-of-pocket Kosten: **$0.00**.")
        P("")
        P("| App | Kosten USD | % vom Total | Was passierte |")
        P("|---|---:|---:|---|")
        notes = {
            "xtts-de-tts": "Größtes Image (PyTorch + TTS + DeepSpeed), 2.3 GB Modell, 124s Cold-Start",
            "orpheus-de-tts": "3 fehlgeschlagene Deploy-Versuche (annotations / SNAC / gated repo)",
            "thorsten-de-tts": "Sauberer Deploy + 1 Cold-Start + 15 Samples",
            "mms-de-tts": "Sauberer Deploy + 1 Cold-Start + 15 Samples",
            "magpie-de-tts": "3× crash-loop (NeMo-Versions-Mismatch), 3 Build-Attempts",
            "voice-agent-de": "Live Push-to-Talk Agent (Whisper + Piper)",
            "piper-de-tts": "Erfolgreich + 2× neu rendert (Voice-Fix kerstin → thorsten)",
        }
        for name, cost in sorted(by_app.items(), key=lambda x: -x[1]):
            pct = (cost/total*100) if total > 0 else 0
            note = notes.get(name, "")
            P(f"| **{name}** | ${cost:.4f} | {pct:.1f}% | {note} |")
        P(f"| **TOTAL** | **${total:.4f}** | 100% | |")
        P("")
        P("### Lessons Learned (Kosten-Hits)")
        P("")
        P("- **86% des Burns floss in 1 App** (XTTS-v2) wegen großem Image + langer Modell-Download. Klein-Modelle wie Piper (~$0.015) und MMS (~$0.029) sind 50× günstiger.")
        P("- **Crash-Loops sind teuer**: Magpie hatte 3 Crash-Loops (~$0.02) ohne dass je ein Sample generiert wurde. **Lesson:** `scaledown_window=2` + Container-Status früh checken, sonst läuft Re-Try-Loop minutenlang.")
        P("- **Gated HF-Repos kostet auch ohne Erfolg**: Orpheus 3 Versuche = $0.12 ohne Output.")
        P("- **`min_containers=0` ist Pflicht** für Pay-per-Use. Wäre `=1` gewesen, hätten 4 GPU-Apps × 24h = ~$240/Tag gekostet.")
        P("")
        P("### Was es gekostet hätte ohne Optimierungen")
        P("")
        P("| Szenario | Geschätzte 24h-Kosten |")
        P("|---|---:|")
        P("| **4 GPU-Apps warm (`min_containers=1`)** | ~$200/Tag (3× L4 @ $0.80/h + 1× L40S @ $1.95/h) |")
        P("| **Voice-Agent immer warm** | ~$1.13/Tag (CPU 4-core × $0.047/h) |")
        P("| **Unser Setup (pay-per-use, alles gestoppt nach Tests)** | **$1.68 für die gesamte Test-Session** |")
        P("")
        P("[Raw billing data](reports/modal_billing_2026-05-18.json)")
        P("")

    P("---")
    P("")
    P("*Generated by [`build_report.py`](client/build_report.py) from `reports/benchmark_results.json` + `reports/modal_billing_2026-05-18.json`.*")
    P("")
